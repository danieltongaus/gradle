...
This release introduces some new properties: `mainClass`, `mainModule`, `modularity` in various places.
Since these are very generic names, there is a chance that you use one of them in your build scripts as variable names.
A new property might then shadow one of your variables in an undesired way, leading to a build failure where a new property is accessed instead of the local variable with the same name.
